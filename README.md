...
3. 配置 LLM：按 `Cmd+,`（macOS）或 `Ctrl+,` 打开设置面板，填写 LLM 提供商和 API Key，保存
4. 启动 Gateway：`npm run dev:gateway`（需要第二个终端）
5. 桌面端 Enter 打开输入框，发送消息即可对话

> **首次使用必须先完成第 3 步**，否则 Gateway 启动会因缺少配置而报错。
